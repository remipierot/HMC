...
public:
	...
	static double eval(double x, double y, double z) {
		...
		if (inSum <= 1) { //We're inside the tetrahedron (3-Simplex) at (0,0,0)
			...
			if (attn0 > 0) {
				attn0 *= attn0;
				value += attn0 * attn0 * OpenSimplexNoise::extrapolate(xsb + 0, ysb + 0, zsb + 0, dx0, dy0, dz0);
			}
...
private:
	static double extrapolate(int xsb, int ysb, int zsb, double dx, double dy, double dz)
	{
	...
	static int perm[256];
	static int permGradIndex3D[256];
	static int gradients3D[72];
};

const double OpenSimplexNoise::STRETCH_CONSTANT_3D = -1.0f / 6.0f;
const double OpenSimplexNoise::SQUISH_CONSTANT_3D = 1.0f / 3.0f;
const double OpenSimplexNoise::NORM_CONSTANT_3D = 103.0f;
int OpenSimplexNoise::perm[] = {};
int OpenSimplexNoise::permGradIndex3D[] = {};
int OpenSimplexNoise::gradients3D[] = {
